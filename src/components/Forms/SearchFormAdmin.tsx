...
const SearchFormAdmin = () => {
    ...
    return (
        <div className="position-relative">
            <input placeholder="Buscar produto" autoComplete="off" className="form-control bg-transparent text-black" type="search" name="search" onChange={searchProduct} />
            {
                productsFound.length > 0 &&
                <div className="form-control position-absolute">
                    {
                        productsFound.map(productFound => (
                            <Link onClick={() => setProductsFound([])} key={productFound.id} className='d-block text-black fw-bold  ps-2 pe-2 pt-1 pb-1' href={{ pathname: `/admin/produto/${productFound.id}`}}>{productFound?.name}</Link>
                        ))
                    }
                </div>
            }
